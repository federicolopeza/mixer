...
def test_load_valid_strategy(tmp_path):
    data = {
        ...
        "bridges": [{"name": "br", "from_chain": "A", "to_chain": "B", "amount_pct": 0.5}],
        "dex_swaps": [{"chain": "A", "router": "R", "path": ["0x1"], "slippage": 0.01}],
        "noise_profile": {"n_micro_txs": {"min": 1, "max": 3}, "contract_pool": ["0x2"]},
        "time_windows": {"active_hours": [0, 23], "weekend_bias": 0.5},
        "distribution": [{"type": "exchange", "destination_address": "0xABC", "amount_bnb": 1.0}],
        "wallets_in_storm": 10,
        "mixing_rounds": 5,
...
@pytest.mark.parametrize("bad_data, error_msg", [
    ...
    ({"noise_profile": {"n_micro_txs": {"min": 5, "max": 2}}}, "n_micro_txs must have min <= max"),
    # Time windows invalid length
    ({"time_windows": {"active_hours": [0], "weekend_bias": 0.5}}, "active_hours"),
    # Distribution missing amounts
    ({"distribution": [{"type": "exchange"}]}, "Must specify amount_bnb or amount_pct"),
])
