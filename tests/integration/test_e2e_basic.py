...
def stub_dependencies(monkeypatch, tmp_path):
    ...
    web3_stub = DummyWeb3()
    monkeypatch.setattr('epic_mixer.core.web3_utils.setup_web3', lambda network: web3_stub)
    monkeypatch.setattr('epic_mixer.core.web3_utils.esperar_deposito', AsyncMock(return_value=1))

    # Stub wallets
    class W:
        def __init__(self, addr): self.address = addr; self.key = "key"
    monkeypatch.setattr('epic_mixer.core.wallets.generar_nuevo_mnemonic', lambda: "seed")
    monkeypatch.setattr('epic_mixer.core.wallets.derivar_wallets', lambda seed, n, idx: [W(f"0x{i}") for i in range(n)])

    # Stub orquestador_epico y exportar_reporte_encriptado
    monkeypatch.setattr('epic_mixer.core.orchestrator.orquestador_epico', AsyncMock(return_value={}))
    monkeypatch.setattr('epic_mixer.utils.reporting.exportar_reporte_encriptado', lambda data, pwd: None)
