...
async def _ejecutar_tormenta_de_mezcla(
    ...
    log.print("Consolidando fondos de la tormenta de vuelta a las wallets de estrategia...")
    tasks = []
    for i, wallet_tormenta in enumerate(wallets_tormenta):
        ...
        if balance > Web3.to_wei('0.001', 'ether'):
            wallet_estrategia_destino = wallets['estrategia'][i % len(wallets['estrategia'])]
            gas_fee_estimada = Web3.to_wei('0.001', 'ether')
...
async def orquestador_epico(
    ...
    try:
        ...
        gas_para_tormenta = Web3.to_wei(config.get('storm_wallet_gas_amount_bnb', 0.002), 'ether')
        tasks = []
        nonce = await web3.eth.get_transaction_count(wallet_deposito.address)
        for i, wallet in enumerate(wallets_tormenta):
            tx_params = {
                'to': wallet.address, 'value': gas_para_tormenta, 'chainId': chain_id
            }
            tasks.append(enviar_transaccion(web3, tx_params, wallet_deposito.key, nonce=nonce+i))
        await asyncio.gather(*tasks)
        log.print(f"[green]Wallets de tormenta fondeadas con {Web3.from_wei(gas_para_tormenta, 'ether')} BNB cada una.")
        await asyncio.sleep(3)
        ...
        for i, leg in enumerate(config['distribution']):
            ...
            if 'amount_bnb' in leg:
                ...
                log.print(f"[red]❌ Pierna de estrategia sin 'amount_bnb' ni 'amount_pct'. Abortando pierna {leg.get('name', i)}.")
                continue
            ...
            log.print(f"Financiando {descrip_monto} desde {wallet_anterior.address[:10]}... a {wallet_actual.address[:10]}...")
            
            tx_params = {
                'to': wallet_actual.address, 'value': monto_wei, 'chainId': chain_id
            }
            await enviar_transaccion(
        ...
        for i, leg in enumerate(config['distribution']):
            ...
            if leg['type'] == 'exchange':
                ...
                tx_params = {
                    'to': destino, 'value': tx_value, 'chainId': chain_id
                }
                await enviar_transaccion(
            ...
            return {'bridges': bridge_hashes, 'dex': dex_hashes, 'merkle_root': merkle_root}
