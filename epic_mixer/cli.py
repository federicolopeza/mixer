...
def gather_user_inputs(config: dict) -> tuple:
    ...
    for leg in config["distribution"]:
        if leg["type"] == "exchange":
            address = Prompt.ask(
                f"Introduce la dirección de depósito para la pierna '[bold yellow]{leg['name']}[/bold yellow]'"
            )
    ...
    num_final_wallets = int(
        Prompt.ask(
            "🔢 ¿A cuántas wallets finales quieres distribuir los fondos?", default="3"
        )
    )
    ...
    if password:
        log.print(
            "[bold green]🔒 Usando contraseña de variable de entorno MIXER_PASSWORD."
        )
    else:
        password = getpass.getpass(
            "🔑 Introduce una contraseña para encriptar el reporte final: "
        )
        if not password:
    ...
    if config.get("time_windows"):
        ...
        log.print(
            f"Ventanas temporales: horas {tw.get('active_hours')} con weekend_bias {tw.get('weekend_bias')}"
        )

    if not Confirm.ask(
        "[bold yellow]¿Iniciar la ejecución de la estrategia?", default=False
    ):
